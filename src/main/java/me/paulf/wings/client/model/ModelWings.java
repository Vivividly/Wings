...
public abstract class ModelWings<A extends Animator> extends Model {
    ...
    static void setAngles(ModelPart left, ModelPart right, Vec3 angles) {
        right.xRot = (left.xRot = Maath.toRadians((float) angles.x));
        right.yRot = -(left.yRot = Maath.toRadians((float) angles.y));
        right.zRot = -(left.zRot = Maath.toRadians((float) angles.z));
    }
}
