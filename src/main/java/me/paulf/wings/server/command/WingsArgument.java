...
public class WingsArgument implements ArgumentType<FlightApparatus> {
    ...
    public FlightApparatus parse(StringReader reader) throws CommandSyntaxException {
        ...
        return Optional.ofNullable(WingsMod.WINGS_REGISTRY.get().getValue(key)).orElseThrow(() -> ERROR_UNKNOWN_WING.create(key));
    }

    @Override
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> ctx, SuggestionsBuilder builder) {
        return SharedSuggestionProvider.suggestResource(WingsMod.WINGS_REGISTRY.get().getKeys(), builder);
    }
